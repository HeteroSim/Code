...
//
    ...
    ctx.data().totalJobs = 0;
    ctx.data().finishedJobs = 0;

    ctx.get<Results>(agent).results = (uint32_t)ctx.data().finishedJobs;
    ctx.get<Results2>(agent).encoded_string[0] = ctx.data().totalJobs;
